...
const PROMPTS: PromptSeed[] = [
  {
    key: "p10", cat: "고객응대", title: "오늘의 한 줄 시장 멘트 만들기",
    ...
    author: "이도현", likes: 40, copies: 62, runs: 35,
    comments: [{ author: "강태양", text: "바로 복사해서 쓰기 딱 좋아요. 결과도 금방 나오고!" }],
  },
  {
    key: "p1", cat: "리서치", title: "종목 리서치 리포트 핵심 요약",
    ...
    author: "박소영", likes: 84, copies: 210, runs: 96, official: true,
    comments: [
  ...
  {
    key: "p2", cat: "고객응대", title: "민원 답변 톤 교정 & 리스크 표현 점검",
    ...
    author: "최민준", likes: 61, copies: 155, runs: 51,
    comments: [
  ...
  {
    key: "p3", cat: "업무보고", title: "주간 업무보고 자동 초안 작성기",
    ...
    author: "권다은", likes: 57, copies: 188, runs: 88,
    comments: [{ author: "정하은", text: "이슈/리스크 분리해주는 게 보고할 때 편해요!" }],
  },
  {
    key: "p4", cat: "리서치", title: "DART 공시 자료 빠른 해설",
    ...
    author: "한지우", likes: 49, copies: 121, runs: 44,
    comments: [{ author: "박소영", text: "정정공시 해석할 때 특히 도움돼요." }],
  },
  {
    key: "p5", cat: "고객응대", title: "펀드·ETF 비교 설명 자료 작성",
    ...
    author: "강태양", likes: 43, copies: 97, runs: 30, comments: [],
  },
  {
    key: "p6", cat: "번역/작성", title: "영문 IR·실적자료 번역 및 요약",
    ...
    author: "윤서연", likes: 38, copies: 84, runs: 41, comments: [],
  },
  {
    key: "p7", cat: "코딩", title: "시세 데이터 조회 SQL 생성기",
    ...
    author: "강태양", likes: 31, copies: 66, runs: 22, comments: [],
  },
  {
    key: "p8", cat: "기획", title: "회의록 정리 & 액션아이템 추출",
    desc: "회의 중 메모를 붙여넣으면 논의 요약·결정사항·액션아이템(담당자/기한)을 자동으로 분리 정리합니다.",
    body: "아래 회의 메모를 바탕으로 정리해주세요.\n\n① 논의 요약\n② 결정 사항\n③ 액션아이템 (담당자 / 기한 포함, 표로)\n④ 다음 회의 안건 후보\n\n메모: {{회의메모}}",
    author: "권다은", likes: 27, copies: 59, runs: 18, comments: [],
  },
  {
    key: "p9", cat: "컴플라이언스", title: "상품 약관 쉬운 설명 변환",
    ...
    author: "최민준", likes: 22, copies: 41, runs: 12, comments: [],
  },
];

// 데모에서 뷰어(권다은)가 좋아요를 누른 프롬프트: id1, id4, id6
const VIEWER_LIKED_PROMPTS = new Set(["p1", "p4", "p6"]);
...
const AGENTS: AgentSeed[] = [
  {
    key: "a101", cat: "리서치", name: "리서치 어시스턴트",
    ...
    author: "박소영", likes: 38, runs: 64,
    comments: [{ author: "강태양", text: "종목 비교를 한 번에 해줘서 상담 준비가 빨라졌어요." }],
  },
  {
    key: "a102", cat: "고객응대", name: "고객 응대 도우미",
    ...
    author: "최민준", likes: 29, runs: 52, comments: [],
  },
  {
    key: "a103", cat: "업무보고", name: "업무 자동화 비서",
    ...
    author: "권다은", likes: 22, runs: 41, comments: [],
  },
];

const VIEWER_LIKED_AGENTS = new Set(["a101"]);
...
async function main() {
  ...
  for (const p of PROMPTS) {
    ...
    if (VIEWER_LIKED_PROMPTS.has(p.key)) {
      await db.like.create({ data: { userId: viewer, promptId: prompt.id } });
    }
  ...
  for (const a of AGENTS) {
    ...
    if (VIEWER_LIKED_AGENTS.has(a.key)) {
      await db.like.create({ data: { userId: viewer, agentId: agent.id } });
    }
  ...
  const notifSeed: { actor: string; type: "like" | "comment"; text?: string; hoursAgo: number }[] = [
    { actor: "박소영", type: "like", hoursAgo: 2 },
    { actor: "정하은", type: "comment", text: "이슈/리스크 분리해주는 게 보고할 때 편해요!", hoursAgo: 26 },
    { actor: "한지우", type: "like", hoursAgo: 27 },
    { actor: "강태양", type: "like", hoursAgo: 50 },
  ];
